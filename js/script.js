function playSound(audioName) {
    ...
    // Check if the browser supports the Promise-based Audio API
    if (typeof audio.play === 'function') {
        audio.play().catch(error => {
            // Handle playback error, if any
            console.error('Error playing audio:', error);
        });
    } else {
        // Fallback for browsers that do not support Promise-based Audio API
        audio.addEventListener('loadeddata', () => {
            audio.play();
        });
    }
}
playSound("music/what can i do.mp3");
